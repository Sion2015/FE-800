...
class Portfolio:
    def __init__(self, data:Data, strategy:BlackLitterman, risk_tolerance, host, account_deposit=100000,
                 if_backtesting=True, frequency="Monthly", drift_limit=0.025, if_benchmark = False):
        ...
        # self.back_testing_start_date = self.data
        # self.back_testing_end_date =
        ...
        if if_backtesting:
            ...
            #self.back_testing_result = self.back_testing_result.join(self.benchmark)
    ...
    def run_back_testing(self, frequency = "Monthly", if_plot = False, if_csv = False):
        price_data = Data(tickers=self.data.tickers,
                          start_date="2016-01-05", # self.data.backtest_start_date,
                          end_date="2017-01-01",  # self.data.backtest_end_date,
                          host=self.host).price
        ...
        self.daily_return = calculate_daily_return(result["daily_total_value"])
        self.annual_return = calculate_annual_return(result["daily_total_value"])
        self.annual_vol = calculate_anual_vol(result["daily_total_value"])
        self.sharpe_ratio = calculate_sharpe_ratio(result["daily_total_value"])

        # print(daily_return)
        # print("annual_return = %f, annual_vol = %f, sharpe_ratio = %f" % (annual_return, annual_vol, sharpe_ratio))
    ...
    def get_portfolio_std(self):
        portfolio_std = np.sqrt((np.transpose(self.target_weights).dot(self.covariance_matrix)).dot(self.target_weights))
        portfolio_std = np.asarray(portfolio_std)
        return portfolio_std

    def get_portfolio_return(self):
        portfolio_return = self.expected_return.dot(self.target_weights)
        return portfolio_return

    def get_historical_return(self):
        historical_return_matrix = self.return_data.dot(self.target_weights)
    ...
    #     df = self.price_data
    #     weights = [1/df.shape[1]]*df.shape[1] # Equally weighted weights
    #     weights = np.matrix(weights).transpose()
    #     shares = np.floor(self.initial_cash * weights / np.matrix(df.iloc[0]).transpose())
    #     residual_cash = self.initial_cash - np.matrix(df.iloc[0]) * shares
    #     residual_cash = float(residual_cash)
    #     benchmark_value = df.dot(shares)
    #     benchmark_value.columns = ['Equally_Weighted_Portfolio']
    #     benchmark_value['Equally_Weighted_Portfolio'] += residual_cash

        #assert isinstance(value, pandas.core.frame.DataFrame)
