...
def calculate_annual_return(price_data):
    stock_return = calculate_daily_return(price_data)
    annual_return = np.mean(stock_return, axis=0) * 252
    return annual_return

def calculate_anual_vol(price_data):
    stock_return = calculate_daily_return(price_data)
    annual_vol = np.std(stock_return, axis=0) * np.sqrt(252)
    return annual_vol

def calculate_sharpe_ratio(price_data, risk_free_rate = 0.75/100):
    stock_return = calculate_daily_return(price_data)
    annual_return = np.mean(stock_return, axis=0) * 252
    annual_vol = np.std(stock_return, axis=0) * np.sqrt(252)
