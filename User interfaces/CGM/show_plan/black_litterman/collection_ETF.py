...
asset_class = ["US STOCKS", "US STOCKS", "US STOCKS", "FOREIGN DEVELOPED STOCKS",
                   ...
                   "NATURAL RESOURCES"]
ETF_dic = dict(zip(ETF24, asset_class))
host = 'localhost:27017'


def add_summary(ETF, posts, N):
...
def build_ETF(ETF, ETF_dic, host='localhost:27017'):
    ...
    for e in range(N):
        ...
        ETFs_data = Share(asset).get_historical(summary_data["inception_date"][asset], today)
        ...
        posts.update_one(
            ...
            {'$set': {'hist_data': [
                {'date': datetime.datetime.strptime(date, '%Y-%m-%d'),
                 'quotes': {'close': float(close), 'volume': float(volume)}}]}}
        ...
        for x in range(1, n):
            ...
            histData = {'$push': {'hist_data': {'date': datetime.datetime.strptime(date, '%Y-%m-%d'),
                                                'quotes': {'close': float(close), 'volume': float(volume)}}}}
...
def get_asset_class(host, ticker=object()):
    client = MongoClient(host=[host])
    db = client.RoboAdvisor
    collection = db.ETF
